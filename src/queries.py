...
use schema test;
create warehouse if not exists tlee_wh_xs warehouse_size = xsmall {"server_type='c6gd2xlarge'" if use_arm_warehouse else ""};
create warehouse if not exists tlee_wh_xxxl warehouse_size = xxxlarge {"server_type='c6gd2xlarge'" if use_arm_warehouse else ""};
...
def create_table_queries(fdn_file_size: Optional[int], name: str, num_rows: int):
    ...
    result += f"""
    create table if not exists {name}(a float, b int);
    delete from {name};
...
def create_test_table_queries(table_name: str, original_table_name: str):
    return f"""
    use warehouse tlee_wh_xs;
    create or replace table {table_name} clone {original_table_name};
    """
...
def _set_clustering_parameters_queries(table_name: str, clustering_parameters: ClusteringParameters):
    ...
    if clustering_parameters.RSO_MEMORY_LIMIT_PARAMETER is not None:
        result += f"alter session set RSO_MEMORY_LIMIT = '{clustering_parameters.RSO_MEMORY_LIMIT_PARAMETER}';"

    if clustering_parameters.MEMORY_SOFT_LIMIT_PERCENT is not None:
...
def setup(session: Session, use_arm_warehouse: bool) -> DataFrame:
    print("\n// ### setup")
    return run_queries(session, setup_queries(use_arm_warehouse))
